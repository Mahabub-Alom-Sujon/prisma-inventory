...
const SupplierCreateUpdate = (props) => {
    ...
    const formSubmit = async (e) => {
        ...
        if(IsEmpty(data.Name)){
            ...
            let url =props.data.id ? `/api/dashboard/suppliers/update?id=${props.id}` : "/api/dashboard/suppliers/create";
            let res=await fetch(url, options);
            let ResJson = await res.json();
            if (ResJson['status'] === "success") {
                setData({id: null,Name:"",Phone:"",Email:"",Address:""}) 
                SuccessToast(`Supplier ${props.data.id ? "Updated" : "Created"}`);
                router.replace("/SupplierList")
                router.refresh()
            }
            else {
                  setSubmit(false);
                ...
                <div className='row justify-content-center'>
                    <div className='col-sm-10 col-md-8 col-lg-5'>
                        <div className='from-box'>
                            ...
                            <h2>{props.data?.id ? "Update" : "Save"} Supplier</h2>
                                 <div className='col-lg-12'>
                                        <input value={data.Name} onChange={(e) => { inputOnChange("Name", e.target.value) }} className="form-control mb-4" type="text" placeholder="Supplier Name" />
                                    </div>
                                     <div className='col-lg-12'>
