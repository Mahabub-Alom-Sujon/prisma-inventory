...
const ReturnCreate = (props) => {
    ...
    useEffect(() => {
        calculateGrandTotal();
    }, [data.products, data.Returns.VatTax, data.Returns.Discount, data.Returns.OtherCost, data.Returns.ShippingCost]);
    ...
    const calculateGrandTotal = () => {
        ...
        const { VatTax, Discount, OtherCost, ShippingCost } = data.Returns;
    ...
    const formSubmit = async (e) => {
        ...
        if (IsEmpty(data.Returns.CustomerID)) {
            ErrorToast("Supplier is required!");
        }
        else if (IsEmpty(data.Returns.VatTax)) {
            ErrorToast("VatTax is required!")
        }
        else if (IsEmpty(data.Returns.Discount)) {
            ErrorToast("Discount is required!")
        }
        else if (IsEmpty(data.Returns.OtherCost)) {
            ErrorToast("OtherCost is required!")
        }
        else if (IsEmpty(data.Returns.ShippingCost)) {
            ErrorToast("ShippingCost is required!")
        }
        else if (IsEmpty(data.Returns.GrandTotal)) {
            ErrorToast("GrandTotal is required!")
        }
        else if (IsEmpty(data.Returns.Note)) {
            ...
            let res = await fetch("/api/dashboard/returns/create", options);
            ...
            if (ResJson['status'] === "success") {
                ...
                SuccessToast("Return Created");
                router.replace("/ReturnList")
                router.refresh()
            }
            else {
                setSubmit(false);
                    ...
                    <div className="col-lg-5 col-md-8 col-sm-10">
                        <div className="from-box">
                            <from className="card animated fadeIn p-5 shadow border-0">
                                <h2 className="purchase">Create Return</h2>
                                <div className="col-lg-12">
                                    <select
                                        value={data.Returns.CustomerID}
                                ...
                                <div className="col-lg-12">
                                    <input
                                        value={data.Returns.ShippingCost}
